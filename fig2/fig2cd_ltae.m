...
[U,~,V]          = svd(randn(Ns,Ns));
Omega            = U * V';
Wpca             = Omega * C(:,1:Ns)'; % this treatment serves to normalize the input
Lpca             = L;
s                = Wpca * input;
s2               = Wpca * input2;
s3               = Wpca * input3;
...
err_s1.em    = zeros(1,NT); % linear TAE empirical
err_s2.th    = zeros(1,NT); % SL theory
err_s2.em    = zeros(1,NT); % SL empirical
err_s3.em    = zeros(1,NT); % PredPCA empirical
...
for h = 1:NT
 ...
 [u1,u2,uc1,E,D] = ltae(s1,s2);
 % u1 ... enoders (training) / prediction
 % u2 ... enoders (test) / prediction
 % uc1 ... enoders (training) / based on current input
 % E ... encoding matrix
 % D ... decoding matrix
 ...
 Q       = (s1*s1(:,[T1,1:T1-1])') * (s1(:,[T1,1:T1-1])*s1(:,[T1,1:T1-1])'+eye(Ns)*prior_s)^(-1); % mapping
 se1     = Q * s1(:,[T1,1:T1-1]);     % input expectations
 se2     = Q * s2(:,[T2,1:T2-1]);     % input expectations
 
 % eigenvalue decomposition
 [C,~,L] = pca(se1');
 Wppca   = C(:,1:Nu)';  % eigenvectors
 Lppca   = L;           % eigenvalues
 u1ppca  = Wppca * se1; % enoders (training) / prediction
 u2ppca  = Wppca * se2; % enoders (test) / prediction
 ...
 % linear TAE
 err_s1.em(1,h) = mean(sum((s2 - D*u2).^2));                       % empirical
 
 % PredPCA
 err_s3.em(1,h) = mean(sum((s2 - Wppca'*u2ppca).^2));              % empirical
 
 %--------------------------------------------------------------------------------
 % system parameter identification
 
 Omega_inv      = (A'*A+eye(Nu)*10^(-4))^(-1)*A'*D;           % ambiguity factor (coordinate transformation)
 [qA,qB,qSigmas,qSigmax,qSigmao,qSigmaz] = calculate_estimated_parameters(s1,u1,uc1,D',Omega_inv,prior_x*T1/T); % this function is for ascending order
...
% save results
csvwrite([dir 'ltae_err_param_' num2str(seed) '.csv'],[1:NT; err_param])
...
% save results
csvwrite([dir 'ltae_err_pred_'  num2str(seed) '.csv'],[1,1,1,-1,1; [err_s0', err_s1.em', err_s2.em', err_s2.th', err_s3.em']/norm_s2])
...
semilogx(t, err_s3.em/norm_s2, '+r')
semilogx(t, err_s1.em(1,:)/norm_s2, '+g'), hold off
axis([0 10^5 0.4 0.8])
