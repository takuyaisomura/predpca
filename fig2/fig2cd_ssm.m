...
[A,B,Sigmas,Sigmap,Sigmao,Sigmaz] = calculate_true_parameters(s3,x3,prior_x); % this function is for ascending order
...
err_s1.em    = zeros(1, NT); % SSM empirical
...
Binit        = BU*BV';
%Binit        = randn(Nx,Nx) * sqrt(trace(B'*B)/Nx/Nx);
...
for h = 1:NT
 ...
 [qxp1,qxc1,qxp2,qxc2,qA,qB] = ssm_kalman_filter(s1,s2,Ainit,Binit,Sigmao,Sigmaz,num_rep,eta,prior_x);
 ...
 % state space model
 err_s1.em(1,h) = mean(sum((s2 - qA*qxp2).^2));                    % empirical
 
 %--------------------------------------------------------------------------------
 % system parameter identification
 
 Omega_inv      = (A'*A+eye(Nu)*10^(-4))^(-1)*A'*qA;               % ambiguity factor (coordinate transformation)
 [qA,qB,qSigmas,qSigmap,qSigmao,qSigmaz] = calculate_estimated_parameters_ssm(s1,qxp1,qxc1,qA,qB,Omega_inv,prior_x*T1/T); % this function is for ascending order
 ...
 err_param(4,h) = norm(Sigmap-qSigmap,'fro')^2 / max(norm(Sigmap,'fro')^2, norm(qSigmap,'fro')^2);
...
% save results
csvwrite([dir 'ssm_err_param_' num2str(seed) '.csv'],[1:NT; err_param])
...
% save results
csvwrite([dir 'ssm_err_pred_'  num2str(seed) '.csv'],[1,1,1,-1; [err_s0', err_s1.em', err_s2.em', err_s2.th']/norm_s2])
