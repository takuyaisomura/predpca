...
test_randomness = 1;
sign_flip       = 1;
[input,input2,input3,label,label2,label3] = create_digit_sequence(dir,sequence_type,T,T2,T3,test_randomness,sign_flip);
...
x2     = generate_true_hidden_states(input2,label2);
x3     = generate_true_hidden_states(input3,label3);
x_mean = mean(x')';
x      = x  - x_mean * ones(1,T);
...
for h = 1:NT
 ...
 err_param(1,h) = norm(A     -qA,     'fro')^2 / norm(A,     'fro')^2;
 err_param(2,h) = norm(B     -qB,     'fro')^2 / norm(B,     'fro')^2;
 err_param(3,h) = norm(Sigmas-qSigmas,'fro')^2 / norm(Sigmas,'fro')^2;
 err_param(4,h) = norm(Sigmax-qSigmax,'fro')^2 / norm(Sigmax,'fro')^2;
 err_param(5,h) = norm(Sigmao-qSigmao,'fro')^2 / norm(Sigmao,'fro')^2;
 err_param(6,h) = norm(Sigmaz-qSigmaz,'fro')^2 / norm(Sigmaz,'fro')^2;
